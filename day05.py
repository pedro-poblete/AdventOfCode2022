...
def create_stacks(stacks_drawing):
    ...
    stacks_drawing.reverse()

    # INITIALIZES STACKS
    stacks = {}
    for i in range(0, len(stacks_drawing[0])):
        stacks[i] = []

    # ADDS ELEMENTS TO THEIR RESPECTIVE STACKS
    for line in stacks_drawing[1:]:
...
def crate_mover_9000(_stacks, _orders):
    for order in _orders:
        for i in range(0, order['quantity']):
            crate = _stacks[order['start']].pop()
            _stacks[order['end']].append(crate)

    # PRINT SOLUTION 1
    for key in _stacks:
        print(_stacks[key][-1], end="")
    print('\n')


def crate_mover_9001(_stacks, _orders):
    for order in _orders:
        crates = _stacks[order['start']][0-order['quantity']:]
        for i in range(0, order['quantity']):
            _stacks[order['start']].pop()

        _stacks[order['end']] = _stacks[order['end']] + crates
...
if __name__ == '__main__':
    inputs = open('input05').read().rstrip().split("\n\n")
    stacks = create_stacks(inputs[0])
    orders = create_orders(inputs[1])
    # SOLUTION 1
