...
def solution(_input):
    matrix = []
    for row in _input.strip().split('\n'):
        matrix.append(list(row))
    trasposition = [[r[_] for r in matrix] for _ in range(len(matrix[0]))]

    def is_visible(icol, irow, value):
        if icol == 0 or irow == 0:
            return True
        if icol == len(matrix[icol])-1 or irow == len(matrix[irow])-1:
            return True
        if all(x < value for x in matrix[irow][:icol]):
            return True
        if all(x < value for x in matrix[irow][icol+1:]):
            return True
        if all(x < value for x in trasposition[icol][:irow]):
            return True
        if all(x < value for x in trasposition[icol][irow+1:]):
            return True
        return False
    ...
    #         if is_visible(i, j, matrix[j][i]):
    #             visible += 1
    # print(visible)

    def calculate_scenic_score(icol, irow, val):
        if icol == 0 or irow == 0:
            return 0
        if icol == len(matrix[icol])-1 or irow == len(matrix[irow])-1:
            return 0
        ...
        pointer = icol
        right = 0
        while pointer < len(matrix[irow])-1:
            if val > matrix[irow][pointer+1]:
                right += 1
                pointer = pointer + 1
            else:
        ...
        top = 0
        while pointer > 0:
            if val > trasposition[icol][pointer-1]:
                top += 1
                pointer = pointer - 1
            else:
                top += 1
        ...
        pointer = irow
        bottom = 0
        while pointer < len(trasposition[icol])-1:
            if val > trasposition[icol][pointer + 1]:
                bottom += 1
                pointer = pointer + 1
            else:
    ...
    for j in range(len(matrix)):
        for i in range(len(matrix[j])):
            new_scenic_score = calculate_scenic_score(i, j, matrix[j][i])
            if new_scenic_score > highest_scenic_score:
                highest_scenic_score = new_scenic_score
    print(highest_scenic_score)


if __name__ == '__main__':
    puzzle_input = open('input08').read()
